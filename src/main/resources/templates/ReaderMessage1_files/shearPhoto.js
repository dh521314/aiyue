...
window.ShearPhoto.prototype = {
    ...
    run: function (ImgUrl, Trues, SetPro) {
        ...
        this.pointhandle(0, 1, "图片已加载，正在截图，请稍后...", 2, "#fbeb61", "#3a414c", function () {
        ...
        this.SendUserMsg("正在为你处理截图，稍等...", 0, 2, "#fbeb61", "#3a414c", true, true, function () {
            ...
            this_.MyAjax.carry({
                ...
                error: function (ErroMsg) {
                    this_.SendUserMsg("错误：连接后端失败，超时或后端无法运行", 5e3, 0, "#f4102b", "#fff", false);
                },
                cache: false
            });
        });
...
window.ShearPhoto.MyAjax.prototype.carry = function (arg) {
    ...
    xmlhttp.setRequestHeader('X-TOKEN', getCookie("pubtoken"));
    ...
    Upsubmit: function (upfile) {
        try {
            ...
            if (this.inArray(filestype.toLowerCase(), this.config.UpType) == -1) {
                ...
                this.config.erro("图片格式一定要是JPG，PNG中的一种");
                window.picDia.hide();
                //传错文件把确认隐藏，关掉弹窗
                // $('#saveShear').hide();
                // setTimeout(function(){
