...
window.ShearPhoto.MINGGE(function () {
    //██████████重要设置████████████████
    //你不要在后面加斜杠，系统会自动给你加上斜杠，index.html的JS引用路径自己改
    ...
    Shear.config({
        ...
        //可选 true,false 。 是否在拖动或拉伸时允许历遍全图（是否让大图动呢）,
        traverse: true,

        /*HTML5重点功能*/
        //是否开启3D移动，CPU加速。可选true  false。默认关闭的，作者认为PC端没必要！在PC端开启后，有部份浏览器页面走位的问题。主要是各大浏览器不统一所致，手机端效果会明显！PC端没什么感觉。 原来是采用left top进行定位的，那么3D移动就是CSS3的translate3d属性。去百度一下translate3D吧
        translate3d: false,

        /*HTML5重点功能*/
        //可选 true,false  是否使用HTML5进行切图 ，支持HTML5浏览器会使用HTML5进行切图，没有HTML5浏览器则采用原始的方式(先上传大图再截取)，SHEARPHOTO这个方案无可挑剔了吧！
        HTML5: true,

        /*HTML5重点功能*/
        //默认请设0 (最大尺寸做事)， HTML上传截图最大宽度， 宽度越大，HTML5截出来的图片容量越大，服务器压力就大，截图就更清淅！ 设得越小 HTML5截出来的图片容量越小.但是造成一定程序的不清淅，请适量设置 当然开启HTML5切图，该设置才有效
        HTML5MAX: 500,

        /*HTML5重点功能*/
        //截好的截图  0至1范围可选（可填小数）   HTML5切图的质量   为1时 最高	，当然开启HTML5切图，该设置才有效,设得越高，越清淅，但文件体积越大，同上！
        HTML5Quality: 0.9,

        /*HTML5重点功能*/
        //如果是HTML5切图时，选择的图片不能超过多少，单位M，,你设大点都不怕，因为HTML5ZIP会对原图进行压缩处理
        HTML5FilesSize: 50,

        /*HTML5重点功能*/
        //是否开启图片特效功能给用户  可选true false,  提示：有HTML5浏览器才会开启的！当然开启HTML5切图，该设置才有效
        HTML5Effects: false,

        /*HTML5重点功能*/
        //HTML5截图前载入的大图 是否压缩图片(数组成员 是数字) ，如果不压缩的话填false，在处理特效时或者拉伸时会明显出卡顿,不流畅！官方强烈建意你进行设置 ，默认填写的是[900,0.9] ,代表宽和高都不能大于900，质量是0.9（最大是1）
        HTML5ZIP: [900, 0.9],

        /*记住 preview (预览图片功能) 尽量设false*/

        // 开启动态预览图片 (数组成员整数型，禁止含小数点 可选false 和数组)     数组内是宽度设置，没有高度设！因为高度会按比例做事 ，此设置代表预览150 大小的预览图（你可以增加多个预览图,如[100,70,50]），设置越多预览图,shearphoto性能越差！官方不建意你开启这个功能，尽可能请设为preview:false
        preview: false,

        //后端处理地址
        // url: relativeUrl + "/ccauthorweb/author/shearphoto_common",
        url:'/ccauthorweb/author/shearphoto_common',

        //可拖动范围宽  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致
        scopeWidth: 256,

        //可拖动范围高  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致
        scopeHeight: 256,
        ...
        /*
         ...
         第3个参数：必须整数！启动后的截框初始宽度(整数型，禁止含小数点)
         第4个参数：比例设置后，这个高度无效，由宽和比例来决定(整数型，禁止含小数点)
         */
        proportional: [1 / 1, 200, 200],

        //截框拉伸或拖拽不能少于多少PX(整数型，禁止含小数点)
        Min: 200,

        //一开始启动时，图片的宽和高，有时候图片会很大的，必须要设置一下(整数型，禁止含小数点)，尽可能和scopeWidth值 一致
        Max: 256,
        ...
        //截框的边框大小 0代表动态边框。大于0表示静态边框，大于0时也代表静态边框的粗细值
        Border: 0,

        //只作用于静态边框，截框的边框类型，其实是引入CSS的border属性，和CSS2的border属性是一样的
        BorderStyle: "solid",
        ...
        UpFun: function () {
            //鼠标健松开时执行函数，把截框现时的宽高告诉JS
            Shear.MoveDiv.DivWHFun();
        }

    });
    /* 截图成功后，返回来的callback */
    //截图成功完成时，由shearphoto.php返回数据过来的成功包
    Shear.complete = function (serverdata) {

        if(serverdata.returnCode == 2000){
            $.lightTip.success(serverdata.returnMsg);
            // setTimeout(function () {
            //     window.location.reload()
            // }, 2000)

            $('.headimg').attr('src', serverdata.result)
            // $('.upload-avatar-popup').hide()
            $("input[name='headimg']").val(serverdata.result);
            window.picDia.hide();
            // $("#inputImage").empty()
        }else{
            $.lightTip.error(serverdata.returnMsg);
        }
        return 
    ...
    //FORM对象
    var ShearPhotoForm = document.getElementById("ShearPhotoForm");
    ShearPhotoForm.UpFile.onclick = function () {
        return false
    };
    //一开始时先不让用户点免得事件阻塞
    var up = new ShearPhoto.frameUpImg({
        //HTML5切图时，不会用到该文件，后端处理地址
        url: relativeUrl + "php/upload.php",

        //FORM对象传到设置
        FORM: ShearPhotoForm,

        //图片类限制，上传的一定只能是图片
        UpType: new Array("jpg", "jpeg", "png"),

        //选择的图片不能超过 单位M（注意：是非HTML5时哦）
        FilesSize: 2,

        //切匆改动这句，不然你他妈又问为什么出错
        HTML5: Shear.HTML5,

        //切匆改动这句 如果是HTML5切图时，选择的图片不能超过 单位M，设太大话，如果客户端HTML5加截超大图片时，会卡爆的
    ...
    try {
        ...
        var photoalbumLifun = function () {
            ...
            //告诉HTML5，图片的类型
            Shear.HTML5.ImagesType = URLType(serveUrl);
            //通过API 接口，加载图片
            Shear.run(serveUrl);
            photoalbum.style.display = "none";
        };
        ...
        //为相册的每张照加入一个点击事件
        for (var i = 0; i < photoalbumLi.length; i++) photoalbumLi[i].onclick = photoalbumLifun;
        //从相册选取事件
        PhotoLoading.onclick = function () {
    ...
    /* 截图，左旋，右旋，重新选择 */
    //按下截图事件，提交到后端的shearphoto.php接收

    Shear.addEvent(document.getElementById("saveShear"), "click", function () {
//我们示例截图并且传参数，后端文件shearphoto.php用 示例：$_POST["shearphoto"] 接收参数，不需要传参数请清空Shear.SendPHP里面的参数示例 Shear.SendPHP();
    ...
    try {
        webcam.init_config = {
            //摄像头像素宽度，也就是说后端生成图片的大小
            width: 200,
            //摄像头像素高度，也就是说后端生成图片的大小
            height: 200,
            //网页中flash的显示宽度
            server_width: 450,
            //网页中flash的显示高度
            ...
            postargs: {mingge: "shearphoto我爱你", shearphoto: "shearphoto好"}
        },

            //拍照连接后端的URL文件，后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
            webcam.set_api_url(relativeUrl + "php/upload.php");
        //拍照是FLASH的，那么你懂的，已经在本版本中帮你加入相对路径，你基本不用改这里了
        webcam.set_swf_url(relativeUrl + "images/webcam.swf");
        //拍照声音，参数1 是否开启， 参数2：声音路径,设置错误，后果是拍照失败，已经在本版本中帮你加入相对路径，你基本不用改这里了
        webcam.set_shutter_sound(true, relativeUrl + "images/shutter.mp3")
        ...
        //按下弹出拍照框事件
        var camerasImageOnclick = function () {
