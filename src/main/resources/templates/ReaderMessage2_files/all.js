...
(function (global, factory) {
    ...
    var Tab = function(el, options) {
        ...
        var defaults = {
            ...
            history: true,
            // 是否短横线滑动, 这个是在原始的luluAPI 的，这里是得到张老师同意添加的一个新接口
            aniLine:true,
            callback: $.noop
        };
        var params = $.extend({}, defaults, options);
            ...
            var moveLine=function(){
                ...
                if(params.aniLine){
                    ...
                    if ($.isFunction(history.pushState)) {
                        line = $it.parent().find('i');
                        if (!line.length) {
                            line = $('<i></i>').addClass('ui-tab-line').prependTo($it.parent());
                        }
                        line.css({
                            display: 'block',
                            width: $it.width(),
        ...
        var _get = function(arr) {
            ...
            $.each(arr, function(index, obj) {
                ...
                if (obj[DISABLED] != true) {
                    html = html + '<a href="' + (obj.href || 'javascript:')+ '"target="' + (obj.target || '_self')  + '" class="' + prefixDropList + 'li' + clSelected + '" data-index="' + index + '" role="option" aria-selected="' + (obj[SELECTED] || 'false') + '" data-target="' + target.attr('id') + '">' + obj.value + '</a>';
                } else {
...
(function (global, factory) {
    ...
    Tips.prototype.show = function(content) {
        ...
        if(this.theme && this.theme !== '') {
            tips.addClass(prefixTips + this.theme);
        }
...
(function (global, factory) {
    ...
    $.lightTip = (function() {
        ...
        return {
            ...
            warning: function(message, time) {
                var lightTips = _create(message).addClass(prefixTips + 'warning');
                lightTips.attr('aria-label', '操作警告');
                setTimeout(function() {
                    lightTips.fadeOut(function() {
                        _remove(lightTips);
                    });
            ...
            normal: function(message, time) {
                var lightTips = _create(message).addClass(prefixTips + 'normal');
                lightTips.attr('aria-label', '操作提示');
                setTimeout(function() {
                    lightTips.fadeOut(function() {
                        _remove(lightTips);
                    });
    ...
    LightTip.prototype.normal = function(message, time) {
        this.el.container = $.lightTip.normal(message, time);

        return this;
    };
    LightTip.prototype.warning = function(message, time) {
        this.el.container = $.lightTip.warning(message, time);
...
(function (global, factory) {
    ...
    // 关闭SVG
    var svg = window.addEventListener ? '<em class="icon-font icon-close">&#xe90e;</em>' : '';
    ...
    var Dialog = function (options) {
        ...
        // 点击蒙层关闭弹窗
        if(params.ismodal) {
            el.container.click($.proxy(function(event) {
                if($(event.target).hasClass(prefixDialog + 'container') && params.ismodal === 'true') {
                    // 关闭弹框
                    this[this.closeMode]();
                }
            }, this));
...
(function (global, factory) {
    ...
    */
        ...
        if (params.data == 'auto') {
            ...
            this.callback.data = function() {
                ...
                ajaxParams.success = function(json) {
                    ...
                    if (json && $.isArray(json.data || json.result)) {
                        self.refresh(self.callback.filter.call(self, json.data || json.result));
                    }
                };
    ...
    Datalist.prototype.refresh = function(data) {
        ...
        if (data && data.length) {
            ...
            $.each(data, function(i, obj) {
                ...
                var placeholder = trigger.attr('placeholder');

                var selected = '';
                if ((value && valueStrip == value) || (value == '' && placeholder && placeholder == valueStrip)) {
                    selected = ' ' + SELECTED;
                    // 这个键盘操作时候需要
                    self.params.index = i;
                }
                ...
                //     self.params.index = i;
                // }
                if (obj.img&&obj.label) {
                    htmlList = htmlList +
                    // 虽然使用其他标签模拟<datalist>
                    // 但是，一些属性值，还是遵循HTML规范
                    '<li class="' + CL + '-option' + selected + '" value="' + valueStrip + '" label="' + labelStrip + '" data-index="' + i + '">' +
                        // label应该前置，可以通过相邻选择器控制后面内容的UI
                        '<img class="' + CL + '-img g-avatar _small" src="'+
                        obj.img +
                        '"' + 'onerror = "this.src=' + '\'https://ccstatic-1252317822.file.myqcloud.com/portraitimg/2019-09-19/5d82ecf354f0d.png\'" >' +
                        '<span class="' + CL + '-value">' +
                            obj.value +
                        '</span><label class="' + CL + '-label">' +
                             obj.label +
                            '</label>' +
                    '</li>';
                } else if (obj.label) {
                    ...
                    '<li class="' + CL + '-option' + selected + '" value="' + valueStrip + '" data-index="' + i + '">' +
                        // label应该前置，可以通过相邻选择器控制后面内容的UI
                        '<a class="' + CL + '-link" href="' + obj.link + '">' +
                        '<span class="' + CL + '-value" title="' + obj.value + '">' +
                            obj.value +
                        '</span></a>' +
                    '</li>';
                } else {
        ...
        if (value && triggerEvent) {
            ...
            this.timerTrigger = setTimeout(function () {
                $.each(triggerEvent, function(i, eventType) {
                    trigger.trigger(eventType, objMatch);
                });
            }, 17);
        ...
        trigger.on({
            ...
            click: function() {
                if (self.display == false) {
                    if (self.params.data == 'auto') {
                        if (self.params.data == 'auto' || $.trim(this.value)) {
                            self.refresh();
                        }
     ...
     */
    Datalist.prototype.position = function(options) {
        // 元素们
        var trigger = this.el.trigger;
        var target = this.el.target;

        if (trigger && target) {
            target.follow(trigger, $.extend({
                // 边缘不自动调整，此处手动调整
                edgeAdjust: false
            }, options || {}));
    ...
    Datalist.prototype.hide = function() {
        ...
        if (target && this.display == true) {
            if (this.params.data != 'auto' && this.value() == '' && trigger.attr('placeholder')) {
                this.value(trigger.attr('placeholder'));
            }
    ...
    */
    ...
    Select.prototype.refresh = function (data) {
        ...
        if (multiple) {
            ...
            })() + '</span><em class="icon-font-arrowdown">&#xe904;</em>');
        }
...
(function (global, factory) {
    ...
    var prefixMinuteCol = 'ui-minute-col-';
    ...
    var DateTime = function(trigger, options) {
        ...
        var defaults = {
            // 文本框初始值，优先从文本框获取。
            value: '',
            // 浮层面板类型，可以是'date'(2015-01-01), 'year'(2015), 'month'(2015-01), 'time'(12:00), 'date-range'(2015-01-01 至 2015-06-01),'minute-col'
            type: 'auto',
        ...
        // 下拉元素
        $('<label for="' + id + '"></label>').addClass(prefixDate + 'arrow icon-font')
            // 插入到文本框的后面
            .insertAfter(input);
        ...
        switch (type) {
            ...
            case 'time': case 'hour': case 'minute': case 'minute-col': {
            ...
            switch (container.attr('data-type')) {
                ...
                case 'date-range': {
                    ...
                    if (/prev|next/.test(this.className)) {
                        ...
                        var isSameAsLast = (function () {
                            var isSame = true;

                            if (range[0].join() != range[1].join()) {
                                isSame = false;
                            } else if (self[SELECTED][0].join() == range[0].join()) {
                                isSame = false;
                            }
                        ...
                        if (self.beforeDateRange && isSameAsLast == false) {
                            self.el.container.find('a[data-type="ensure"]').click();
                        }
                ...
                case 'minute-col': {
                    ...
                    if (/item/.test(this.className)) {
                        ...
                        if (type == 'hour') {
                            self[SELECTED][0] = this.innerHTML;
                        } else if (type == 'minute') {
                            self[SELECTED][1] = this.innerHTML;
                        }
    ...
    DateTime.prototype.val = function() {
        ...
        switch (this.type) {
            ...
            case 'time':  case 'minute': case 'minute-col': {
                input.val(selected.join(':'));
                break;
            }
            case 'hour': {
    ...
    DateTime.prototype['date-range'] = function() {
        ...
        if (this.beforeDateRange) {
            html += this.beforeDateRange();
        }
     ...
     * @return {Object} 返回当前实例对象
     */
    DateTime.prototype['minute-col'] = function() {
        var arrTime = this[SELECTED];

        // 文本框元素比较常用，使用局部变量，一是节约文件大小，二是性能更好！
        var input = this.el.input;
        // input.prop("readonly", false); // 设置为可输入类型
        // 分钟时间间隔, 默认为5分钟
        var step = input.attr('step') * 1 || 1;
        // 选择分钟的完整HTML
        // 1. 外部的容器，含有专有容器类名，可以重置内部的一些样式
        var html = '<div class="' + prefixMinuteCol + 'x">';
        // 2. 头部小时的左右切换
        var hour = arrTime[0] * 1;
        // //   首先是公共部分
        // html = html + '<div class="' + prefixDate + 'head">';
        ...
        // 3. 分钟主体
        html = html + '<div class="' + prefixMinuteCol + 'body">' + (function() {
            var htmlMinuteCol = '';
            var hour = '';
            var minute = '';
            var cl = '';
            ...
            // hour列 0～23
            htmlMinuteCol = htmlMinuteCol + '<div class="' + prefixMinuteCol + 'panel' + '">' + '<div class="' + prefixMinuteCol + 'con' + '">';
            for (var i = 0; i < 24; i += step ) {
                hour = i + '';
                if (hour.length == 1) {
                    hour = '0' + hour;
                ...
                // hour列
                htmlMinuteCol = htmlMinuteCol + '<a href="javascript:" class="' + cl + '" data-type="hour">' + hour + '</a>'
                // minute列
            }
            htmlMinuteCol = htmlMinuteCol + '</div>';

            // minute列 0～59
            htmlMinuteCol = htmlMinuteCol + '<div class="' + prefixMinuteCol + 'con' + '">';
            for (var i = 0; i < 60; i += step ) {
                minute = i + '';
                if (minute.length == 1) {
                    minute = '0' + minute;
                ...
                // hour列
                htmlMinuteCol = htmlMinuteCol + '<a href="javascript:" class="' + cl + '" data-type="minute">' + minute + '</a>'
            }
            htmlMinuteCol = htmlMinuteCol + '</div>' + '</div>';

            return htmlMinuteCol;
        })() + '</div>';

        // 容器闭合标签
        html += '</div>';

        this.el.container.attr('data-type', 'minute-col').html(html);
    ...
    Pagination.prototype.create = function(num, mode) {
        ...
        if (mode == 'long') {
            ...
            if (total <= visible) {
                ...
                html = html + '<span class="' +  clEll + '"><em class="icon-font">&#xe923;</em></span>';
                long(total);
...
(function (global, factory) {
    ...
    Form.prototype.ajax = function() {
        ...
        xhr.open(form.attr('method') || 'GET', form.attr('action'), true);
        ...
        xhr.onload = function () {
            ...
            if (json.code == 0 || json.returnCode == 1000000 || json.error == 0) {
                // 下一步
                optionCallback.success.call(form, json);
            } else {
                $.lightTip.error(json.returnMsg || '您的操作没有成功，你可以稍后重试。');
                // 回调
                if ($.isFunction(optionCallback.error)) {
                    optionCallback.error.apply(form, json);
                }
...
/**
 * 默认初始化一些基础方法「 只适用于直出 」
 * 如果是动态渲染的需要还是用 lulu 的new 的方式 手动初始化
 */
...
(function(){    
     ...
     * 有这个 j_tabs 的会默认渲染
     */
    new Tab($('.j_tabs > a.ui-tab-tab'));

    /**
     * 有这个 .j_pagination 会基于dom 初始化 分页
     * 这个只适用于直出，如果是动态渲染的还是采用 new 方式
     */
    $('.j_pagination').each(function(){
        var $it=$(this);
        new Pagination($it, {
            length: $it.attr('data-length'),
            every: $it.attr('data-every'),
            current: $it.attr('data-current') || 1
        });
    });    

    /**
     * select 标签上如果有这个class 会自动初始化
     */
    $('.j_select').selectMatch();
})();